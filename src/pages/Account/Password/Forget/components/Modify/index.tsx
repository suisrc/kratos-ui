...
const DefaultView: React.FC<{
  ...
  useEffect(() => {
    if (dispatch && !rules?.length) {
      dispatch({ type: 'forgetPassword/fetchUserPasswordRules' });
    }
  }, []);
  ...
  const onChangePassword = async () => {
    ...
    if (dispatch) {
      dispatch({
        type: 'forgetPassword/submitNewPassword',
...
export default connect(({ forgetPassword }: { forgetPassword: StateType }) => ({
  signature: forgetPassword.signature,
  warn: forgetPassword.warnData,
  rules: forgetPassword.passwordRules,
}))(DefaultView);
